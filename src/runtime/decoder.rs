...
impl Decoder {
  ...
  pub(crate) fn step(
    ...
    // Reject NaN/Inf at the session boundary. The sampler's
    // all-non-finite check (sampler.rs) only fires when EVERY logit
    // is non-finite — a single NaN slips through, then poisons
    // softmax (NaN spreads through `e^x` and the sum) and biases
    // argmax (`f32::total_cmp` orders NaN as the largest). Catch it
    // at the boundary so callers see `SessionNonFiniteOutput`
    // instead of an arbitrary token.
